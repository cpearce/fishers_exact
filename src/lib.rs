...
/// `TooLargeValueError` is returned by `fishers_exact` when the input observed
/// table contains values too large. Currently "too large" is values greater than
/// `std::i32::MAX`. Note that for large sample sizes tests other than Fisher's exact
/// test of are more appropriate to determine statistical independence.
#[derive(Clone, Copy, Debug)]
pub struct TooLargeValueError;

impl fmt::Display for TooLargeValueError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "all integers passed to fishers_exact must be less than {}", std::i32::MAX)
    }
}

impl Error for TooLargeValueError {
...
pub fn fishers_exact(table: &[u32; 4]) -> Result<FishersExactPvalues,TooLargeValueError> {
    if table.iter().any(|&x| x > std::i32::MAX as u32) {
        return Err(TooLargeValueError{})
    }
    Ok(exact22(table[0] as i32, table[1] as i32, table[2] as i32, table[3] as i32))
}
